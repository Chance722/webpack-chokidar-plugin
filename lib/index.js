...
wepackChokidarPlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('done', function (compilation) {
    ...
    watcher.on('add', function (path) {
      options.onAddCallback ? (options.onAddCallback(path), watcher.close()) : function () {
        return null;
      }();
    }).on('change', function (path) {
      options.onChangeCallback ? (options.onChangeCallback(path), watcher.close()) : function () {
      ...
      options.onUnlinkCallback ? (options.onUnlinkCallback(path), watcher.close()) : function () {
        return console.log("File ".concat(path, " has been removed"));
      }();
    });
    watcher.on('addDir', function (path) {
      options.onAddDirCallback ? (options.onAddDirCallback(path), watcher.close()) : function () {
        return console.log("Directory ".concat(path, " has been added"));
      }();
    }).on('unlinkDir', function (path) {
      options.unlinkDirCallback ? (options.unlinkDirCallback(path), watcher.close()) : function () {
        return console.log("Directory ".concat(path, " has been removed"));
      }();
    }).on('error', function () {
      options.onErrorCallback ? (options.onErrorCallback(), watcher.close()) : function (error) {
      ...
      options.onRawCallback ? (options.onRawCallback(event, path, details), watcher.close()) : function () {
        return null;
      }();
    });
  });
